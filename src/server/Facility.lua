...
local function highBay(parent, top, lensY, color, shadows, bright)
	bright = bright or 4.2
	...
	pointLight(emit, 34, 0.7 * bright / 4.2, color)
end
			...
			for j = 0, nz - 1 do
				...
				if not (r.Id == "Atrium" and x * x + z * z < 14 * 14) then
					if r.Lamp == "Dome" then
						...
						highBay(parent, Vector3.new(x, y, z), y - 7.5, r.Warm or rgb(255, 232, 200), (i + j) % 2 == 0, r.LampBright)
					end
				end
			end
		end
...
local function buildHangar(parent)
	...
	-- a spare Sentinel head on a maintenance stand: a steel pedestal with a
	-- turned collar, the suit's own helm on top (Costumes.SentinelHead),
	-- cables hanging out of its neck and a work lamp over it
	local hp = Vector3.new(40, F, 90)
	local steel, dark = rgb(70, 74, 82), rgb(34, 36, 40)
	P(parent, Vector3.new(4.4, 0.4, 4.4), CFrame.new(hp + Vector3.new(0, 0.2, 0)), M.DiamondPlate, dark)
	P(parent, Vector3.new(2.2, 2.6, 2.2), CFrame.new(hp + Vector3.new(0, 1.7, 0)), M.Metal, steel)
	drum(parent, hp + Vector3.new(0, 3.1, 0), 3.2, 0.3, M.Metal, dark)
	local headModel = Costumes.SentinelHead(parent, CFrame.lookAt(hp + Vector3.new(0, 5.2, 0), hp + Vector3.new(-1, 5.2, 0)), 2.6, "Default")
	for _, d in headModel:GetDescendants() do
		if d:IsA("BasePart") then
			d.CanQuery, d.CanTouch = false, false
		end
	end
	for k = -1, 1 do
		cable(hp + Vector3.new(k * 0.4, 3.3, 0.3), hp + Vector3.new(k * 1.6 + 2, 0.3, 1.8), 0.8, 0.18, k == 0 and rgb(160, 30, 30) or rgb(22, 22, 24))
	end
	cyl(parent, hp + Vector3.new(2.6, 0.4, 0), hp + Vector3.new(2.6, 9, 0), 0.2, M.Metal, dark, nil, true) -- work lamp stand
	cyl(parent, hp + Vector3.new(2.6, 9, 0), hp + Vector3.new(0.8, 9.4, 0), 0.16, M.Metal, dark, nil, true)
	local lamp = drum(parent, hp + Vector3.new(0.6, 9.2, 0), 1.2, 0.5, M.Metal, dark, nil, true)
	spotDown(drum(parent, hp + Vector3.new(0.6, 8.94, 0), 0.9, 0.04, M.Neon, rgb(170, 200, 255), nil, true), 14, 1.6, rgb(170, 200, 255), 60)
	_ = lamp
...
local function buildSurgery(parent)
	...
	for k, off in { Vector3.new(-3, -5.6, -1.6), Vector3.new(2.8, -5.2, 1.8) } do
		...
		D(parent, Vector3.new(0.5, 2.9, 2.9), aim * CFrame.new(0, 0.2, 0) * CFrame.Angles(0, 0, math.rad(90)), M.SmoothPlastic, white, { Shape = Enum.PartType.Cylinder })
