...
### Recent rules
- **Pounce** only works while Wolverine is running on all fours (hold C / Ctrl). It's a dive that peaks about 5.5 studs up, so it stays under the 11-stud doorways, and covers the same ~72 studs as the old high leap. If it catches nobody, it ends in a landing crouch the moment he touches down (`Config.Abilities.Pounce.Height` / `AirTime`).
